...
impl LinkedListAllocator {
    ...
    pub unsafe fn alloc_with_boundary(&self, size: usize, align: usize, boundary: usize) -> *mut u8 {
        ...
        let ptr = unsafe { self.alloc_with_boundary_unchecked(size, align, boundary) };
        if ptr.is_null() {
            panic!("oom");
        }
        let us = ptr as usize;
    ...
    pub unsafe fn alloc_with_boundary_unchecked(&self, size: usize, align: usize, boundary: usize) -> *mut u8 {
        ...
        if boundary == 0 {
            return unsafe { self.alloc(core::alloc::Layout::from_size_align_unchecked(size, align)) }
        }
...
impl SimplestAllocator {
    ...
    pub unsafe fn alloc_with_boundary(&self, layout: Layout, boundary: usize) -> *mut u8 {
        ...
        let ptr = unsafe { self.alloc_with_boundary_unchecked(layout, boundary) };
        let us = ptr.addr();
