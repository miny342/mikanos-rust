...
#[no_mangle]
extern "efiapi" fn kernel_main_new_stack(config: *const FrameBufferConfig, memmap_ptr: *const uefi::mem::memory_map::MemoryMapOwned) -> ! {
    unsafe {
        (*config).frame_buffer.write_bytes(123, (*config).horizontal_resolution * (*config).vertical_resolution * 4);
    }
